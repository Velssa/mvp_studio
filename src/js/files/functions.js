...
export let _fadeIn = (target, duration = 500) => {
	if (!target.classList.contains('_slide')) {
		target.classList.add('_slide');
...
export let _fadeOut = (target, duration = 500) => {
	if (!target.classList.contains('_slide')) {
		target.classList.add('_slide');
		if (target.hidden) {
			target.hidden = false;
	...
	if (spollersArray.length > 0) {
		...
		function setSpollerAction(e) {
			...
			if (el.hasAttribute('data-spoller') || el.closest('[data-spoller]')) {
				...
				if (!spollersBlock.querySelectorAll('._slide').length) {
					...
					if(spollerTitle.closest('[data-spollitem]')){
						spollerTitle.closest('[data-spollitem]').classList.toggle('_spollitem-active');
					}					
		...
		function hideSpollersBody(spollersBlock) {
			...
			if (spollerActiveTitle) {
				...
				spollerActiveTitle.closest('[data-spollitem]').classList.remove('_spollitem-active');
		...
		if (tabsContent.length > 0) {
			tabsContent.forEach((tabsContentItem, index) => {
				if (tabsTitles[index].classList.contains('_tab-active')) {
					if (tabsBlockAnimate) {
						_fadeOut(tabsContentItem, tabsBlockAnimate);
					} else {
					...
					if (tabsBlockAnimate) {
						_fadeIn(tabsContentItem, tabsBlockAnimate);
					} else {
